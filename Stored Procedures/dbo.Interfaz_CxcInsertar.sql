...
AS
	...
	-- Se crea y se genera Xml con los parametros para tabla Interfaz_Logs
		...
		-- Se Registra Evento
        EXEC Interfaz_LogsInsertar 'Interfaz_CxcInsertar', 'Inserción', '', @Usuario, @LogParametrosXml;
	...
	--********************************************************************
		/* Validaciones Comunes*/
                SELECT  @sError = VE.fn_ValidaInterfaz(@Empresa, @Mov, @FechaEmision, @Concepto, @Moneda, @TipoCambio,
                                                       @Usuario, @Cliente, @Sucursal, @Vencimiento, @Importe, @Impuestos,
                                                       @CentroDeCostos, @TipoPago);

                IF NOT EXISTS ( SELECT  ctad.CtaDinero
                                FROM    dbo.CtaDinero ctad
                                WHERE   ctad.CtaDinero = RTRIM(@CtaDinero) )
                    AND @Mov IN ( 'Cobro VE Gravado', 'Cobro TransInd' )
                    BEGIN
                        SET @sError = 'La cuenta indicada ( ' + ISNULL(@CtaDinero, 'No Indicada')
	...
	--Evaluar Retirarlo
                ...
                IF ( @Mov IN ( 'Cobro SIVE', 'Cobro TransInd', 'Cobro VE Gravado', 'Cobro Paquete', 'Bonificación TI',
                    ...
                    BEGIN
                        IF ( @Aplica IS NULL
                             OR RTRIM(LTRIM(@Aplica)) = ''
                           )
                            BEGIN
                                SET @sError = 'Aplica no indicado. En Movimientos de ' + RTRIM(@Mov)
                                    + ' debe indicar un movimiento Aplica. ' + 'Por favor, indique un Aplica.';
                            END
				
                        IF ( @AplicaID IS NULL
                             OR RTRIM(LTRIM(@AplicaID)) = ''
                           )
                            BEGIN
                                SET @sError = 'AplicaID no indicado. En movimientos de ' + RTRIM(@Mov)
                                    + ' debe indicar un AplicaID. ' + 'Por favor, indique un AplicaID.';
                            END
				
                        IF ( SELECT COUNT (*) FROM @T_Partidas
                           ) <> 1
                            BEGIN
                                SET @sError = 'De acuerdo al movimiento indicado, solo puede indicar una partida. Por favor, indique solo una partida valida.';
                            END
                        ...
                        IF NOT EXISTS ( SELECT  *
                            ...
                            BEGIN
                                SET @sError = 'Factura no encontrada. Los parámetros Aplica = "' + RTRIM(@Aplica)
                                    + '" y AplicaID = "' + RTRIM(@AplicaID)
                                    + '" hacen referencia a una factura que no fue encontrada. '
                        ...
                        IF ( ( SELECT TOP 1
                            ...
                            BEGIN
                                SET @sError = 'Factura no se encuentra "PENDIENTE". Los parámetros Aplica = "'
                                    + RTRIM(@Aplica) + '" y AplicaID = "' + RTRIM(@AplicaID)
                        ...
                        IF ( ( SELECT TOP 1
                            ...
                            BEGIN
                                SET @sError = 'La factura no corresponde al cliente indicado. Los parámetros Aplica = "'
                                    + RTRIM(@Aplica) + '" y AplicaID = "' + RTRIM(@AplicaID)
                        ...
                        IF ( ( SELECT TOP 1
                            ...
                            BEGIN
                                SET @sError = 'El Importe es superior al Saldo de la factura (Saldo igual a '
                                    + CAST(( SELECT TOP 1
                                                    A.Saldo
                                             FROM   Cxc AS A
                                             WHERE  A.Mov = @Aplica
                            ...
                            END
                        IF ( SELECT CAST( SUM (Importe) AS MONEY) FROM @T_Partidas
                           ) <> ( @Importe + @Impuestos )
                            BEGIN
                                SET @sError = 'Importe de aplicación no valido. El importe de aplicación indicado es diferente al importe del movimiento. '
                ...
                IF EXISTS ( SELECT  *
                            FROM    @T_Partidas AS T1
                                    LEFT JOIN Cxc AS T2 ON T1.AplicaID = T2.MovID
                        ...
                        SET @sError = @sError + ( SELECT TOP 1
                                                            ( SELECT    CAST(T2.AplicaID AS VARCHAR) + ','
                                                              FROM      @T_Partidas AS T2
                                                                        LEFT JOIN Cxc AS T3 ON T2.AplicaID = T3.MovID
	...
	--********************************************************************
                SET @bMovValido = 0;
	----	***	'Bonificación SIVE' ***
 --       IF ( @Mov = 'Bonificación SIVE' )
 --           BEGIN
 --               SET @bMovValido = 1;
 --               IF ( @Usuario = 'SITTI' )
 --                   BEGIN
 --                       IF ( @Concepto NOT IN ( 'CANC. ING. VE CFD', 'CANCELACION INGRESO VE',
 --                                               'DEV/SOBRE VENTAS DE VIAJES ESPECIALES', 'VIAJES ESPECIALES' ) )
 --                           BEGIN
 ...
 --                           END
 --                       IF ( @Aplica <> 'CFD FactVia Credito' )
 --                           BEGIN
 --                               SET @sError = 'Aplica no valido. El movimiento Aplica se valida de acuerdo al movimiento y usuario indicado. '
 --                                   + 'Por favor, indique un Aplica valido para esta combinación de movimiento y usuario.';
 --                           END
 --                   END
 --               ELSE
 --                   BEGIN
 --                       SET @sError = 'Usuario no valido. El usuario que indico existe en Intelisis, '
 ...
 --               IF ( @Usuario = 'SITTI' )
 --                   BEGIN
 --                       IF ( @Concepto NOT IN ( 'COBRO VIAJES ESPECIALES', 'VIAJES ESPECIALES' ) )
 --                           BEGIN
 --                               SET @sError = 'Concepto no valido. El concepto se valida de acuerdo al movimiento y usuario indicado. '
 --                                   + 'Por favor, indique un Concepto valido para esta combinación de movimiento y usuario.';
 --                           END
 --                       IF ( @Aplica NOT IN ( 'CFD FactVia Credito', 'CFD Sin Viaje', 'Fact Via Credito' ) )
 --                           BEGIN
 --                               SET @sError = 'Aplica no valido. El movimiento Aplica se valida de acuerdo al movimiento y usuario indicado. '
 --                                   + 'Por favor, indique un Aplica valido para esta combinación de movimiento y usuario.';
 --                           END
                        ...
                        IF ( @Usuario = 'SITTI' )
                            BEGIN
                                ...
                                IF ( @Aplica NOT IN ( 'FACT.VE.GRAVADO', 'CFDI SIN VIAJE GRAV', 'Nota Cargo SIVE',
                                                      'CFD FactVia Credito', 'CFD Sin Viaje', 'Factura TraspInd',
                                                      'Factura INE', 'Endoso', 'Factura VE TOTAL', 'INE VE GRAVADO' ) )
                                    BEGIN
                                        SET @sError = 'Aplica no valido. El movimiento Aplica se valida de acuerdo al movimiento y usuario indicado. '
                                            + 'Por favor, indique un Aplica valido para esta combinación de movimiento y usuario.';
                                    END
 ...
 --               IF ( @Usuario = 'SITTI' )
 --                   BEGIN
 --                       IF ( SELECT A.Concepto
 --                            FROM   Cxc AS A
 --                            WHERE  A.MovID = @AplicaID
 --                                   AND A.Empresa = @Empresa
 --                          ) IN ( 'VIAJES COBRADOS POR ANTICIPADO', 'VIAJES COBRADOS X ANT.' )
 --                           BEGIN
 --                               SET @Concepto = 'DEV/SOBRE VENTAS DE VIAJES ESPECIALES'
 --                           END
 --                       IF ( @Concepto NOT IN ( 'CFD VIAJES COBRADOS X ANT.', 'DEV/SOBRE VENTAS DE VIAJES ESPECIALES',
 --                                               'VIAJES COBRADOS POR ANTICIPADO', 'ANTICIPO PROMOCIONES EXENTA',
 --                                               'CFD ANTICIPO PROMOCIONES EXENTA', 'CFD ANTICIPO PROMOCION GRAVADA' ) )
 --                           BEGIN
 --                               SET @sError = 'Concepto no valido. El concepto se valida de acuerdo al movimiento y usuario indicado. '
 --                                   + 'Por favor, indique un Concepto valido para esta combinación de movimiento y usuario.';
 --                           END
 --                       IF ( @Aplica <> 'CFD Anticipo' ) --If exists (Select * From #partidas where Aplica <> 'CFD Anticipo')
 --                           BEGIN
 --                               SET @sError = 'Aplica no valido. El movimiento Aplica se valida de acuerdo al movimiento y usuario indicado. '
 --                                   + 'Por favor, indique un Aplica valido para esta combinación de movimiento y usuario.';
 --                           END
 --                       IF ( @Impuestos <> 0 )
 --                           BEGIN
 --                               SET @sError = 'Este movimiento no debe tener Impuestos. El movimiento Devolución debe indicar Impuestos 0 '
 ...
 --           END	
	-- ***	'Devolucion Gravada' ***
                IF ( @Mov = 'Devolucion Gravada' )
                    BEGIN
                        ...
                        IF ( @Usuario = 'SITTI' )
                            BEGIN
                                ...
                                IF ( @Impuestos <> 0 )
                                    BEGIN
                                        SET @sError = 'Este movimiento no debe tener Impuestos. El movimiento Devolución Gravada debe indicar Impuestos 0 '
                                            + 'en el encabezado. Por favor, indique un valor de Impuestos valido para este movimiento.';
                                    END
                    ...
                    END	                               
	---- ***	'Devolucion Paquete' ***
 --       IF ( @Mov = 'Devolucion Paquete' )
 --           BEGIN
 --               SET @bMovValido = 1;
 --               IF ( @Usuario = 'SITTI' )
 --                   BEGIN
 --                       IF ( @Concepto <> 'ANTICIPO PROMOCION GRAVADA' )
 --                           BEGIN
 --                               SET @sError = 'Concepto no valido. El concepto se valida de acuerdo al movimiento y usuario indicado. '
 --                                   + 'Por favor, indique un Concepto valido para esta combinación de movimiento y usuario.';
 --                           END
 --                       IF ( @Aplica <> 'CFD Anticipo ServCom' )
 --                           BEGIN
 --                               SET @sError = 'Aplica no valido. El movimiento Aplica se valida de acuerdo al movimiento y usuario indicado. '
 --                                   + 'Por favor, indique un Aplica valido para esta combinación de movimiento y usuario.';
 --                           END
 ...
 --           END	                                   
	---- ***	'Devolucion Saldo' ***
 --       IF ( @Mov IN ( 'Devolucion Saldo', 'DEV.SALDO' ) )
 --           BEGIN
 --               SET @bMovValido = 1;
 --               IF ( @Usuario = 'SITTI' )
 --                   BEGIN
 ...
 --           END	
	---- ***	'Nota Cargo SIVE' ***
 --       IF ( @Mov = 'Nota Cargo SIVE' )
 --           BEGIN
 --               SET @bMovValido = 1;
 --               IF ( @Usuario = 'SITTI' )
 --                   BEGIN
 --                       IF ( @Concepto NOT IN ( 'CLIENTES V.E.ESP. GRAVADOS', 'T.INDUSTRIAL 15%' ) )
 --                           BEGIN
 --                               SET @sError = 'Concepto no valido. El concepto se valida de acuerdo al movimiento y usuario indicado. '
 --                                   + 'Por favor, indique un Concepto valido para esta combinación de movimiento y usuario.';
 --                           END
 ...
 --           END	
	----	***	'Nota Credito SIVE' ***
 --       IF ( @Mov = 'Nota Credito SIVE' )
 --           BEGIN
 --               SET @bMovValido = 1;
 --               IF ( @Usuario = 'SITTI' )
 --                   BEGIN
 --                       IF ( @Concepto <> 'CANC. ING. VE CFD' )
 --                           BEGIN
 --                               SET @sError = 'Concepto no valido. El concepto se valida de acuerdo al movimiento y usuario indicado. '
 --                                   + 'Por favor, indique un Concepto valido para esta combinación de movimiento y usuario.';
 --                           END
 ...
 --                   END
 --           END	
	----	***	'Nota Credito TransIn' ***
 --       IF ( @Mov = 'Nota Credito TransIn' )
 --           BEGIN
 --               SET @bMovValido = 1;
 --               IF ( @Usuario = 'SITTI' )
 --                   BEGIN
 --                       IF ( @Concepto <> 'CANC. INGRESO T.IND. 15%'
 --                            AND @Concepto <> 'T.INDUSTRIAL 10%'
 --                            AND @Concepto <> 'T.INDUSTRIAL 11%'
 --                            AND @Concepto <> 'T.INDUSTRIAL 15%'
 --                            AND @Concepto <> 'T.INDUSTRIAL 16%'
 --                          )
 --                           BEGIN
 --                               SET @sError = 'Concepto no valido. El concepto se valida de acuerdo al movimiento y usuario indicado. '
 --                                   + 'Por favor, indique un Concepto valido para esta combinación de movimiento y usuario.';
 ...
 --                   END
 --           END	
	----	***	'NOTA CREDITO VE GRAV' ***
 --       IF ( @Mov = 'NOTA CREDITO VE GRAV' )
 --           BEGIN
 --               SET @bMovValido = 1;
 --               IF ( @Usuario = 'SITTI' )
 --                   BEGIN
 --                       IF ( @Concepto <> 'CANCELACION VE GRAVADO' )
 --                           BEGIN
 --                               SET @sError = 'Concepto no valido. El concepto se valida de acuerdo al movimiento y usuario indicado. '
 --                                   + 'Por favor, indique un Concepto valido para esta combinación de movimiento y usuario.';
 --                           END
 ...
 --           END	
		--	***	'CANCELACION TURISMO' ***
                IF ( @Mov IN ( 'CANCELACION TURISMO' ) )
                    BEGIN
                        ...
                        IF ( @Usuario = 'SITTI' )
                            BEGIN
                                IF ( @Concepto NOT IN ( 'CANCELACION VE GRAVADO', 'CANC. INGRESO T.IND. 15%',
                                                        'T.INDUSTRIAL 16%' ) )
                                    BEGIN
                                        SET @sError = 'Concepto no valido. El concepto se valida de acuerdo al movimiento y usuario indicado. '
                                            + 'Por favor, indique un Concepto valido para esta combinación de movimiento y usuario.';
                                    END
                                IF ( SELECT TOP 1 CAST( Importe AS MONEY) FROM @T_Partidas
                                   ) < ( @Importe + @Impuestos )
                                    BEGIN
                                        SET @sError = 'Importe de aplicación no valido. El importe de aplicación indicado es menor al importe del movimiento. '
                                            + 'Por favor, indique un Concepto valido para esta combinación de movimiento y usuario.';
                                    END
                            END
                        ELSE
                ...
                IF @sError <> 'Ok'
                    BEGIN
                        EXEC Interfaz_LogsInsertar 'Interfaz_CxcInsertar', 'Error de Validación', @sError, @Usuario,
                            @LogParametrosXml;
	...
	--		PROCESO
	--********************************************************************
		/* dividiendo al cadena de referencia */
        --SET @Ref0 = SUBSTRING(@Referencia, 1, 50)
        --IF LEN(@Referencia) > 50
        --    BEGIN
        --        SET @Ref1 = SUBSTRING(@Referencia, 51, 50)
        --        IF LEN(@Referencia) > 100
        --            BEGIN
        --                SET @Ref2 = SUBSTRING(@Referencia, 101, 50)
        --                IF LEN(@Referencia) > 150
        --                    BEGIN
        --                        SET @Ref3 = SUBSTRING(@Referencia, 151, 50)
        --                        IF LEN(@Referencia) > 200
        --                            BEGIN
        --                                SET @Ref4 = SUBSTRING(@Referencia, 201,
        --                                                      50)
        --                            END
        --                    END
        --            END
	...
	--********************************************************************
		/*Revisar con Jose Salinas si se va necesitar 
        IF @Mov IN ( 'Cobro VE Gravado', 'Cobro TransInd' )
            BEGIN
                UPDATE  dbo.Cte
                SET     ClaveBanco = @CveBanco ,
                        CtaBanco = @CtaBanco
                WHERE   Cliente = @Cliente;
            END */
	...
	--	*****	SE INSERTAN LOS REGISTROS EN CXC	*****
                ...
                IF ( @Mov NOT IN ( 'DEV.SALDO', 'Devolucion', 'Devolucion Gravada', 'Devolucion Saldo',
                                   'Devolucion Paquete', 'Cobro SIVE', 'Cobro Paquete', 'Cobro TransInd',
                                   'Cobro VE Gravado', 'CANCELACION TURISMO', 'Bonificación TI', 'BONIFICACION VE GRAV' ) )
                    BEGIN
                        SET @AplManual = 0;
                        SET @Origen = NULL;
                    END
        ...
        IF EXISTS ( SELECT  c.Codigo
            ...
            BEGIN
                BEGIN TRY
                    ...
                    --INSERT  INTO #afectar
                    EXEC spAfectar 'CXC', @RegresoID, 'AFECTAR', 'Todo', NULL, @Usuario, NULL, 1, @iError OUTPUT,
                        @sError OUTPUT;
                ...
                IF ( SELECT A.Estatus FROM Cxc AS A WHERE A.ID = @RegresoID
                   ) = 'SINAFECTAR'
                    BEGIN
                        SET @sError = 'Error al aplicar el movimiento de Intelisis: ' + 'Error = '
                            + CAST(ISNULL(@iError, -1) AS VARCHAR(255)) + ', Mensaje = ' + ISNULL(@sError, '')
                            + '. Intente nuevamente.';
                        EXEC Interfaz_LogsInsertar 'Interfaz_CxcInsertar', 'Error', @sError, @Usuario, @LogParametrosXml;
                        RAISERROR(@sError,16,1);
                        RETURN;
                    END
            END
		
        SET @RegresoMovID = ( SELECT A.MovID FROM Cxc AS A WHERE A.ID = @RegresoID
                            );
        SET @sError = NULL;
        ...
        IF EXISTS ( SELECT  c.CFDFlexEstatus
            ...
            BEGIN
                ...
                BEGIN TRY
                    ...
                    EXEC Interfaz_LogsInsertar 'Interfaz_CxcInsertar', 'Timbrado CFDI', @sError, @Usuario,
                        @LogParametrosXml;
                END TRY
                BEGIN CATCH
                    SELECT  @iError = ERROR_NUMBER() ,
	...
	--********************************************************************
                IF EXISTS ( SELECT  c.Codigo
                    ...
                    BEGIN
                        BEGIN TRY
                            EXEC spAfectar 'CXC', @a2ID, 'AFECTAR', 'Todo', NULL, @Usuario, NULL, 1, @iError OUTPUT,
                                @sError OUTPUT;
                            SELECT  @sError = ml.Descripcion
                            FROM    dbo.MensajeLista ml
                            WHERE   ml.Mensaje = @iError;
                        ...
                        IF ( SELECT A.Estatus FROM Cxc AS A WHERE A.ID = @a2ID
                           ) = 'SINAFECTAR'
                            BEGIN
                                SET @sError = 'Error al aplicar el movimiento de aplicación 2 de Intelisis: '
                                    + 'Error = ' + CAST(ISNULL(@iError, -1) AS VARCHAR(255)) + ', Mensaje = '
                                    + ISNULL(@sError, '') + ', el movimiento fue cancelado. Intente nuevamente.';
                                EXEC Interfaz_LogsInsertar 'Interfaz_cxcInsertar', 'Error', @sError, @Usuario,
                                    @LogParametrosXml;
                                RAISERROR(@sError,16,1);
                                RETURN;
                            END			
        ...
        IF ( SELECT c.Estatus
            ...
            AND @Mov IN ( 'Cobro VE Gravado', 'Cobro TransInd' )
            AND NOT EXISTS ( SELECT Documento
    ...
    --                    @OkRef = @sError OUTPUT; -- varchar(max)
                ...
                IF NOT EXISTS ( SELECT  cpt.Documento
                ...
                IF @Mov IN ( 'Cobro VE Gravado', 'Cobro TransInd' )
                    BEGIN
                        ...
                        SELECT  ID = @RegresoID ,
                                T2.MovID ,
                                T2.Estatus ,
                                ( CASE WHEN T1.Documento IS NOT NULL THEN 'CONCLUIDO'
                                       ELSE ISNULL(@sError, 'Error')
                                  END ) AS CFDFlexEstatus ,
                                CFDXML = CAST(T1.Documento AS VARCHAR(MAX)) ,
                                T1.noCertificado ,
                                T1.Sello ,
                        ...
                        FROM    dbo.CFDICobroParcialTimbrado AS T1
                                RIGHT JOIN dbo.Cxc T2 ON T2.ID = T1.IDModulo
                                                         AND T2.MovID = T1.MovID
                        ...
                        SELECT  @ID = @RegresoID ,
                                @MovID = @RegresoMovID ,
                                @Estatus = T2.Estatus ,
                                @CFDFlexEstatus = ( CASE WHEN T1.Documento IS NOT NULL THEN 'CONCLUIDO'
                                                         ELSE ISNULL(@sError, 'Error')
                                                    END ) ,
                                @CFDXml = CAST(T1.Documento AS VARCHAR(MAX)) ,
                                @noCertificado = T1.noCertificado ,
...
GO
GRANT EXECUTE ON  [dbo].[Interfaz_CxcInsertar] TO [Linked_Svam_Pruebas]
GO
