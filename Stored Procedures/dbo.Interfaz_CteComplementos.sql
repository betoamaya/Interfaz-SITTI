...
--********************************************************************
        IF @Usuario IS NULL
            OR NOT EXISTS ( SELECT  u.Usuario
        ...
        IF ISNULL(@Sucursal, 0) <> 0
            BEGIN
                IF NOT EXISTS ( SELECT  cea.Cliente ,
                                        cea.ID
                    ...
                    BEGIN
                        SELECT  @iError = -1 ,
                                @sDescr = 'La sucursal indicada no existe para el cliente ' + RTRIM(@Cliente)
                    ...
                    BEGIN
                        IF @Tipo = 2 /* C O M P L E M E N T O --- I N E*/
                            BEGIN
                                ...
                                IF ( SELECT ISNULL(tp.Dato01, '') AS 'TipoProceso'
                                    ...
                                    BEGIN
                                        SELECT  @iError = -1 ,
                                                @sDescr = 'Complemento INE - Tipo de Proceso Invalido. Por favor, indique Tipo de Proceso validó.';
                                        EXEC dbo.Interfaz_LogsInsertar @SP = 'Interfaz_CteComplementos', -- varchar(255)
                                            @Tipo = 'Error de Validación', -- varchar(255)
                                            @DetalleError = @sDescr, -- varchar(max)
                                            @Usuario = @Usuario, -- varchar(10)
                        ...
                        --        IF ( SELECT RTRIM(tp.Dato02) AS 'Tipo de Comite'
                        --             FROM   @T_Parametros AS tp
                        --             WHERE  tp.Cliente = @Cliente
                        --                    AND tp.Sucursal = @Sucursal
                        --           ) = 'Ejecutivo Nacional'
                        --            BEGIN
                        --                PRINT 'Tipo de Comité Ejecutivo Nacional'
                        --                UPDATE  @T_Parametros
                        --                SET     Dato04 = NULL ,
                        --                        --Dato05 = NULL ,
                        --                        Dato06 = NULL
                        ...
                        --                   ) = 'Ejecutivo Estatal'
                        --                    BEGIN
                        --                        PRINT 'Tipo de Comité Ejecutivo Estatal'
                        --                        UPDATE  @T_Parametros
                        --                        SET     Dato03 = NULL
                        --                        WHERE   Cliente = @Cliente
                        --                                AND Sucursal = @Sucursal;
                        ...
                        --                    BEGIN
                        --                        SELECT  @iError = -1 ,
                        --                                @sDescr = 'Complemento INE - Tipo de Comité Invalido. Por favor, indique Tipo de Comité validó.';
                        --                        EXEC dbo.Interfaz_LogsInsertar @SP = 'Interfaz_CteComplementos', -- varchar(255)
                        --                            @Tipo = 'Error de Validación', -- varchar(255)
                        --                            @DetalleError = @sDescr, -- varchar(max)
                        --                            @Usuario = @Usuario, -- varchar(10)
                        ...
                        --    BEGIN
                        --        SELECT  @iError = -1 ,
                        --                @sDescr = 'El valor de Tipo incorrecto. Se esperaba un valor de tipo diferente a '
                        --                + RTRIM(ISNULL(@Tipo, 0)) + '. Por favor, indique Tipo validó.';
                        --        EXEC dbo.Interfaz_LogsInsertar @SP = 'Interfaz_CteComplementos', -- varchar(255)
                        --            @Tipo = 'Error de Validación', -- varchar(255)
                        --            @DetalleError = @sDescr, -- varchar(max)
                        --            @Usuario = @Usuario, -- varchar(10)
...
--********************************************************************
        ...
        IF @Tipo = 1 /* A D E N D A --- F E M S A*/
            BEGIN
                IF NOT EXISTS ( SELECT  cod.Cliente
                ...
                BEGIN TRY
                    ...
                    FROM    dbo.Cte AS c
                            INNER JOIN @T_Parametros AS tp ON tp.Cliente = c.Cliente
                    ...
                    FROM    dbo.CteOtrosDatos AS cod
                            INNER JOIN @T_Parametros AS tp ON tp.Cliente = cod.Cliente
                    WHERE   cod.Cliente = @Cliente;
                END TRY
                BEGIN CATCH
                    ...
                    BEGIN
                        BEGIN TRY
                            ...
                            FROM    dbo.CteCFD AS cc
                                    INNER JOIN @T_Parametros AS tp ON tp.Cliente = cc.Cliente
                ...
                ELSE
                    BEGIN
                        BEGIN TRY
                            ...
                            FROM    dbo.CteEnviarA AS cea
                                    INNER JOIN @T_Parametros AS tp ON tp.Cliente = cea.Cliente
                            WHERE   cea.Cliente = @Cliente
                                    AND cea.ID = @Sucursal;
                        END TRY
                        BEGIN CATCH
